    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="sidebar-container flex flex-col">
            ...
            <div class="sidebar-bottom">
                <a href="{{ route('shop.index') }}" class="sidebar-link text-gray-600 hover:text-green-600">
                    <i class="fas fa-external-link-alt sidebar-icon"></i>
                    <span>Visit Store</span>
                </a>
                <form action="{{ route('logout') }}" method="POST" class="sidebar-link text-gray-600 hover:text-red-600 cursor-pointer">
                    @csrf
                ...
                <div class="px-6 py-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <h1 class="text-2xl font-semibold text-gray-800">@yield('page-title', 'Admin Panel')</h1>
                                ...
                                <div class="relative group">
                                    <button class="flex items-center space-x-3 pl-4 border-l border-gray-200 hover:border-indigo-300 transition-colors">
                                        <div class="text-right">
                                            <p class="text-sm font-medium text-gray-900 group-hover:text-indigo-600 transition-colors">{{ Auth::user()->name }}</p>
                                            <p class="text-xs text-gray-500 group-hover:text-indigo-500 transition-colors">
                                                {{ Auth::user()->isAdmin() ? 'Administrator' : (Auth::user()->isManager() ? 'Manager' : 'User') }}
                                            </p>
                                        </div>
                                    ...
                                    <div class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                        <div class="py-2">
                                            <a href="{{ route('profile') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
