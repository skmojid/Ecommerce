...
<nav class="bg-gray-100 py-3">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ol class="flex items-center space-x-2 text-sm text-gray-500">
            <li>
                <a href="{{ route('shop.index') }}" class="hover:text-gray-700">
            ...
            <li class="text-gray-900 font-medium">{{ $category->name ?? 'Category' }}</li>
        </ol>
    </div>
</nav>
                    ...
                    @foreach($subcategories as $subcategory)
                        <a href="{{ route('shop.category', $subcategory) }}" 
                           class="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-200 hover:border-indigo-300 group">
                            ...
                            @if($subcategory->products_count ?? $subcategory->products->count())
                                <p class="text-sm text-gray-500 mt-1">
                                    {{ $subcategory->products_count ?? $subcategory->products->count() }} products
                                </p>
                            @endif
                        </a>
                    @endforeach
                        ...
                        <div class="relative">
                            ...
                            <div class="aspect-w-1 aspect-h-1 bg-gray-100 overflow-hidden">
                                @if($product->primaryImage && $product->primaryImage->first())
                                    <img src="{{ asset($product->primaryImage->first()->image_path) }}" 
                                         alt="{{ $product->name }}" 
            ...
            @empty
                <div class="col-span-full text-center py-16">
                    <div class="bg-white rounded-lg shadow-sm p-12">
                        ...
                        <a href="{{ route('shop.categories.index') }}" class="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
